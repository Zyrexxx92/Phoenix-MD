  ...
  start: async (
    ...
    switch (inputCMD) {
      ...
      case "setgcname":
        ...
        try {
          ppgc = await Phoenix.profilePictureUrl(m.from, "image");
        } catch {
          ppgc = botImage1;
        }

        await Phoenix.groupUpdateSubject(m.from, text)
          .then((res) =>
            Phoenix.sendMessage(
              m.from,
              {
      ...
      case "del":
        ...
        if (!isBotAdmin) {
          ...
          await doReact("📛");
          await Phoenix.sendMessage(m.from, { delete: key });
        } else {
          if (!isAdmin) {
            await doReact("❌");
            return m.reply(
      ...
      case "demote":
        ...
        await doReact("📉");
        try {
          await Phoenix.groupParticipantsUpdate(m.from, [userId], "demote").then(
            (res) =>
              Phoenix.sendMessage(
                m.from,
                {
      ...
      case "grouplink":
        ...
        await doReact("🧩");
        let link = await Phoenix.groupInviteCode(m.from);
      ...
      case "gc":
        ...
        if (text === "close") {
          await Phoenix.groupSettingUpdate(m.from, "announcement").then((res) =>
            m.reply(`Group has been closed!`)
          );
        } else if (text === "open") {
          await Phoenix.groupSettingUpdate(m.from, "not_announcement").then(
      ...
      case "promote":
        ...
        await doReact("💹");
        try {
          await Phoenix.groupParticipantsUpdate(m.from, [userId], "promote").then(
            (res) =>
              Phoenix.sendMessage(
                m.from,
                {
      ...
      case "remove":
        ...
        await Phoenix.groupParticipantsUpdate(m.from, [users], "remove").then(
          (res) =>
            Phoenix.sendMessage(
              m.from,
              {
      ...
      case "setppgc":
        ...
        let quotedimage = await Phoenix.downloadAndSaveMediaMessage(quoted);
      ...
      case "setgcdesc":
        ...
        await Phoenix.groupUpdateDescription(m.from, newGCdesc).then((res) =>
          Phoenix.sendMessage(
            m.from,
            {
              image: { url: ppgc, mimetype: "image/jpeg" },
      ...
      case "revoke":
        ...
        if (m.from == "120363040838753957@g.us") {
          await doReact("❌");
          return m.reply(
            "Sorry, this command is not allowed in *Phoenix Support Group* !\n\nYou are not allowed to change support group link !"
          );
        }
        await doReact("💫");

        await Phoenix.groupRevokeInvite(m.from).then((res) =>
          Phoenix.sendMessage(
            m.from,
