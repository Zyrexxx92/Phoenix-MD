...
module.exports = {
  ...
  start: async (Phoenix, m, { inputCMD, text, doReact, prefix, pushName }) => {
    switch (inputCMD) {
      case "play":
      case "song":
        if (!text) {
        ...
        await doReact("📥");
        thumbPhoenix = "https://graph.org/file/d0a287fa875c809f234ce.jpg";
...
_🏮 Video Uploaded:_ *${song.ago}*\n`,
        ...
        YT.mp3(videoId).then((file) => {
          ...
          ffmpeg(inputPath)
            ...
            .on("end", async () => {
              const thumbnailBuffer = await getBuffer(thumbPhoenix);
...
_🏮 Video Uploaded:_ *${song.ago}*\n`,
      ...
      case "ytsearch":
        ...
        Phoenix.sendMessage(m.from, buttonMessage, { quoted: m });
        break;
