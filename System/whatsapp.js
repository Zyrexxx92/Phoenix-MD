...
exports.serialize = (Phoenix, m, options = {}) => {
  if (!m) return m;
  ...
  if (m.key) {
    ...
    m.sender = jidNormalizedUser(
      (m.fromMe && Phoenix.user?.id) || m.key.participant || m.from || ""
    );
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe =
        m.quoted.sender == jidNormalizedUser(Phoenix.user && Phoenix.user?.id);
      m.quoted.text =
        m.quoted.msg?.text ||
      ...
      m.quoted.delete = () =>
        Phoenix.sendMessage(m.quoted.from, { delete: vM.key });
      m.quoted.download = (pathFile) =>
        Phoenix.downloadMediaMessage(m.quoted.msg, pathFile);
    }
  }
  m.download = (pathFile) => Phoenix.downloadMediaMessage(m.msg, pathFile);
  m.body = m.text =
  ...
  m.reply = (text, chatId = m.from, options = {}) =>
    Buffer.isBuffer(text)
      ? Phoenix.sendFile(chatId, text, "file", "", m, { ...options })
      : Phoenix.sendText(chatId, text, m, { ...options });
