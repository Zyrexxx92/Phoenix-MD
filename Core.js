...
module.exports = async (Phoenix, m, commands, chatUpdate) => {
  try {
    ...
    const metadata = m.isGroup ? await Phoenix.groupMetadata(from) : {};
    ...
    global.suppL = "https://chat.whatsapp.com/KSM8yCpBHGUGkb2f3zfGz7";
    ...
    if (isAntilinkOn && m.isGroup && !isAdmin && !isCreator && isBotAdmin) {
      const linkgce = await Phoenix.groupInviteCode(from);
